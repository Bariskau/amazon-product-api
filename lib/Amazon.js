...
class AmazonScraper {
    ...
    httpRequest({uri, method, qs, json, body, form}) {
        let amzn = new URL(uri ? `${this.mainHost}/${uri}` : this.mainHost);

        for (const query in qs) {
            amzn.searchParams.set(query, qs[query])
        }

        let scraper = new URL('http://api.scrape.do')

        scraper.searchParams.set('token', this.token)
        scraper.searchParams.set('url', amzn.href)
        scraper.searchParams.set('CustomHeaders', true)
    ...
    async prepareContent(item = 1) {
        ...
        if (this.scrapeType === 'products') {
            if (!this.totalProducts) {
                let totalResultCount = body.match(/"totalResultCount":(\d+)/gm);
                if (totalResultCount) {
                    this.totalProducts = totalResultCount[0].split('totalResultCount":')[1];
                }
            }

            if (!this.asinOnPageCount) {
                let asinOnPageCount = body.match(/"asinOnPageCount":(\d+)/gm);
                if (asinOnPageCount) {
                    this.asinOnPageCount = asinOnPageCount[0].split('asinOnPageCount":')[1];
                }
            }
    ...
    async mainLoop() {
        if (this.scrapeType === 'products') {
            ...
            this.asyncPage = Math.ceil(this.number / this.asinOnPageCount)
        }

        return new Promise((resolve, reject) => {
            forEachLimit(
                Array.from({length: this.asyncPage}, (_, k) => k + (this.scrapeType === 'products' ? 2 : 1)),
                this.asyncTasks,
                async (item) => {
                    await this.prepareContent(item)
                },
                (err) => {
    ...
    grabReviews(body) {
        ...
        for (let key in scrapingResult) {
            ...
            try {
                const rating = /\d+(:?,|\.)\d+/.exec(search[0].children[0].children[0].data)[0].replace(',', '.')
                scrapingResult[key].rating = parseFloat(rating);
            } catch (error) {
                continue;
            }
        }
    ...
    grapAsinDetails(body) {
        ...
        try {
            output.reviews.rating = $(`span.reviewCountTextLinkedHistogram.noUnderline`)[0]
                ? /\d+(:?,|\.)\d+/.exec($($(`span.reviewCountTextLinkedHistogram.noUnderline`)[0]).text())[0].replace(',', '.')
                : 0;
            output.reviews.rating = parseFloat(output.reviews.rating)
        } catch {
            // continue regardless of error
        }
        ...
        output.price.discounted = $(`span.a-price[data-a-strike=true] > span.a-offscreen`)[0] ? true : false;

        output.price.before_price = output.price.discounted
            ? this.geo.price_format($(`span.a-price[data-a-strike=true] > span.a-offscreen`).text())
    ...
    grabProduct(body, p) {
        ...
        for (let key in scrapingResult) {
            try {
                const priceSearch = $(`div[data-asin=${key}] span[data-a-size="l"]`)[0] || $(`div[data-asin=${key}] span[data-a-color="base"]`)[0];
    ...
    validateRating(item) {
        if (this.scrapeType === 'products') {
            if (!item.reviews.rating && this.minRating === 1 && this.maxRating === 5) {
                return item
            }
    ...
    extractImages($, body) {
         ...
         */
        if (!images.length) {
            const imageData = $('#imgBlkFront')[0] || $('#ebooksImgBlkFront')[0] || $('#landingImage')[0];
            if (imageData) {
